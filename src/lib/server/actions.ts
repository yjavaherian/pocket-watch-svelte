...
export async function getProject(projectID: number) {
  ...
  const computedProject = {
    ...
    sessions: _project.sessions.map((s) => ({
      ...s,
      date: new Date(
        Math.min(...s.blocks.map((b) => new Date(b.start).getTime())),
      ),
  ...
  return {
    ...computedProject,
    duration: computedProject.sessions.reduce(
      (acc, s) => acc + (s.duration ?? 0),
      0,
    ),
    invoiceTotal:
      computedProject.sessions.reduce((acc, s) => acc + (s.duration ?? 0), 0) *
      (computedProject.billingRate ?? 0),
    startDate: new Date(
      Math.min(...computedProject.sessions.map((s) => s.date.getTime())),
    ),
    endDate: new Date(
      Math.max(...computedProject.sessions.map((s) => s.date.getTime())),
    ),
  };
}
...
export async function startNewSession(projectID: number, desc: string) {
  const new_seesion = (
    await db.insert(session).values({ projectID, desc }).returning()
  )[0];
  ...
  return await db.query.session.findFirst({
    where: eq(session.id, new_seesion.id),
    with: { blocks: true },
  });
}
...
export async function createNewBlock(
  ...
  const new_block = (
    await db.insert(block).values({ start, end, sessionID }).returning()
  )[0];
  return await db.query.block.findFirst({ where: eq(block.id, new_block.id) });
}
...
export async function stopSession(sessionID: number) {
  ...
  if (_session.status == "active") {
    ...
    await db
      ...
      .where(eq(block.id, last_block.id));
  }
  await db
    .update(session)
    .set({ status: "stopped" })
